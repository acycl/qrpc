...
// the wire frame to minimize allocations. This method is typically called by
// generated client stubs rather than directly by application code.
func (cc *ClientConn) Invoke(ctx context.Context, method string, req, reply proto.Message) error {
	...
	// Cancel the stream if the caller's context is done. This ensures that
	// context deadlines and cancellation propagate to in-flight stream I/O.
	...
	respBytes, err := readResponse(stream)
	if err != nil {
		return err
	}

	if err := proto.Unmarshal(respBytes, reply); err != nil {
